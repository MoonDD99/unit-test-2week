...
public class MockServiceTest {
    ...
    @Test
    public void 어떠한포지션을저장해도미드리턴테스트(){
        Champion champion = mock(Champion.class);
        champion.setPosition("미드");
        when(champion.getPosition()).thenReturn("미드"); // setPosition하면 return값이 없어 error
    ...
    public void shouldOneTimeRunForChampionArraySize(){
        ...
        mockChampion.add(midChamp);

        System.out.println("size :: " + mockChampion.size()); //가짜이기때문에 size는 0이된다.
        verify(mockChampion, times(1)).size();
    }

    // 4-1. champion 객체에서 이름을 가져오는 로직이 2번 이상 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldTwoTimeRunForChampionName(){
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
        ...
        System.out.println("topChampion's name :: " + topChamp.getName());
        System.out.println("midChampion's name :: " + topChamp.getName());//가짜이기때문에 size는 0이된다.
    ...
    @Test
    public void shouldOneTimeFunFroChampionGetName(){
        Champion champion = mock(Champion.class);
        champion.setName("조이");
        champion.setPosition("미드");
        champion.setHasSkinCount(3);
    ...
    public void List객체사이즈검증테스트(){
        ...
        champions.add(new Champion("Temp", "Mid", 1));
        champions.add(new Champion("Temp1", "Mid", 2));
        champions.add(new Champion("Temp2", "Mid", 3));
